...
function X509KeyInfo(certificatePath, publicKeyPath) {
    this.getKeyInfo = function (key, prefix) {
        var cert = fs.readFileSync(publicKeyPath).toString();
        var certInfo = cert.replace('-----BEGIN CERTIFICATE-----', '').replace('-----END CERTIFICATE-----', '').replace(/\s/g, '').replace(/(\r\n\t|\n|\r\t)/gm, '');
        prefix = prefix || '';
        prefix = prefix ? prefix + ':' : prefix;
        return "<" + prefix + "X509Data><X509Certificate>" + certInfo + "</X509Certificate></" + prefix + "X509Data>";
    };
    this.getKey = function (keyInfo) {
        var certPath = path.resolve(certificatePath);
        return fs.readFileSync(certPath);
    };
}
function computeSignedRequest(requestName, requestXml, certificatePath, publicKeyPath) {
