...
export class CloudAdapter {
    ...
    private static async createRequest(options: {method: string, host: string; path?: string; port?:number; protocol?:string, headers?:Record<string, string>}, body?: any): Promise<string> {
        ...
        return new Promise<any>((resolve, reject) => {
            ...
            if  (body) {
                console.log('cloud-adapter', body);
                req.write(data);
            }
            req.end();
        });
    ...
    async getALFCertificateDate(): Promise<Date> {
        const uploadDate = await this.getEntity('Certificate')[0].uploadDate;
    ...
    async fileDownload(fileName:  string, directory: string): Promise<string> {
        const certificateSignedUrl = await CloudAdapter.createRequest({ method: 'get', host: 'api.portal.dynamicsmobile.com', path: `/storage/file2/${this.appArea}/${fileName}?root=${directory}`, 
    ...
    async fileUpload(fileName:  string,  fileContent: string, directory: string) {
        let fileSignedUrl: any = await CloudAdapter.createRequest({ method: 'post', host: 'api.portal.dynamicsmobile.com', path: `/storage/file2/${this.appArea}/${fileName}?root=${directory}`, 
    ...
    async addEntity(entityName: string, entityValue: string)  {
            await CloudAdapter.createRequest({ method: 'post', host: 'api.portal.dynamicsmobile.com', path: `/livelink/entity/${this.appArea}/ALF/${entityName}/add`, 
            headers: { 'x-api-key': this.apiKey, 
    ...
    async getEntity(entityName: string) {
        await CloudAdapter.createRequest({ method: 'post', host: 'api.portal.dynamicsmobile.com', path: `/livelink/entity/${this.appArea}/ALF/${entityName}/query`, 
        headers: { 'x-api-key': this.apiKey, 
        'Content-Type':'application/json',
        } });
    }
